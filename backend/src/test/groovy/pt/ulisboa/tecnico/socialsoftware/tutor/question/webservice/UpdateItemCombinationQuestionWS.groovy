...
class UpdateItemCombinationQuestionWS extends SpockTest {
    @LocalServerPort
    private int port
    ...
    def "update an item combination question with two items and one connection with a teacher"() {
        ...
        def items = new ArrayList<ItemDto>()
        def itemOneDto = new ItemDto()
        itemOneDto.setContent(ITEM_2_CONTENT)
        def itemTwoDto = new ItemDto()
        itemTwoDto.setContent(ITEM_1_CONTENT)
        items.add(itemOneDto)
        items.add(itemTwoDto)
        questionDto.getQuestionDetailsDto().setItems(items)
